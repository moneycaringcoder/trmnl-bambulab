...
/v1/user-service/user/sendemail/code` with `{ email, type: "codeLogin" }` sends
the code without a prior password attempt, and `POST
/v1/user-service/user/login` with `{ account, code }` returns a token. The Bambu
portion of hosted enrolment ran on that path: code requested, code accepted, and
printers listed. Confidence **confirmed**.
